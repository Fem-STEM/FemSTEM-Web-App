...
ceye.addEventListener("click", function () {
  cEyeView = !cEyeView;
  if (cEyeView) {
    ceye.setAttribute("class", "fa fa-eye");
    document.getElementById("cpassword").setAttribute("type", "text");
  } else {
    ceye.setAttribute("class", "fa fa-eye-slash");
...
!(function ($) {
  ...
  Multiselect.prototype = {
    ...
    templates: {
      button:
        '<button type="button" class="multiselect dropdown-toggle form-control" data-toggle="dropdown"></button>',
    ...
    buildButton: function () {
      // Build button.
      this.$button = $(this.templates.button).addClass(
      ...
      $("li input", this.$ul).on(
        ...
        $.proxy(function (event) {
          ...
          var isSelectAllOption =
            $(event.target).val() == this.options.selectAllValue;
    ...
    createOptionValue: function (element) {
      ...
      //Hack by Victor Valencia R.
      if (
        $(element).parent().hasClass("multiselect-icon") ||
        $(element).parent().parent().hasClass("multiselect-icon")
      ) {
    ...
    buildSelectAll: function () {
      var alreadyHasSelectAll = this.$select[0][0]
        ? this.$select[0][0].value == this.options.selectAllValue
        : false;
    ...
    rebuild: function () {
      ...
      $(
        'option[value="' + this.options.selectAllValue + '"]',
        this.$select
      ).remove();
    ...
    getSelected: function () {
      return $(
        'option[value!="' + this.options.selectAllValue + '"]:selected',
        this.$select
      ).filter(function () {
